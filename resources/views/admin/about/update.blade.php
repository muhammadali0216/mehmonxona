...
@section('content')
  <form class="row gx-3 gy-2 align-items-center" style="margin: 5px;" action="{{ route('about.update', $id=$info->id) }}" method="POST" enctype="multipart/form-data" >
    ...
    <div class="col-sm-3">
        ...
        <input type="text" name="titleEn" class="form-control" id="specificSizeInputName"value="{{ $info->titleEn }}">
        <label class="visually-hidden" for="specificSizeInputName">Sarlavxa Ru</label>
        <input type="text" name="titleRu" class="form-control" id="specificSizeInputName"value="{{ $info->titleRu }}">
        @error('titleUz')
        <p class="qizil">{{ $message }}</p>
        @enderror
    </div>
